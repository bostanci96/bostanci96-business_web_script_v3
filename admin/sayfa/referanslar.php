...
<?php
	...
	if($id!=2 && $id!=5){
		die('error');
			...
			$aciklama = "Projelerinizi bu alanda yönetebilirsiniz.";
			$h2 	= "Mevcut Projeleriniz";
											...
											<td>
												<div class="btn-group dropdown">
													...
													<ul class="dropdown-menu danger" role="menu">
														<li><a href="index.php?sayfa=referans_duzenle&id=<?php echo $slideRow["fotograf_id"];?>">Görüntüle / Düzenle</a></li>
														<li>
														...
														<li><a href="javascript:;" onclick="TekSil(<?php echo $slideRow["fotograf_id"];?>);">Projeyi Sil</a></li>
													</ul>
												</div>
											</td>
										</tr>
...
function TekSil(slideId){
	$.post('ajax.php?p=tek_foto_sil', {id: slideId}, function (data) {
		if(data=="basarili"){
			sweetAlert({
				title	: "Başarılı",
				text 	: "Proje başarılı bir şekilde silindi.",
				type	: "success"
			},
			function(){
...
function durumDegis(slideId){
	$.post('ajax.php?p=foto_durum_degis', {id: slideId}, function (data) {
		if(data=="yasaklama-basarili"){
			sweetAlert({
				title	: "Başarılı",
				text 	: "Proje başarılı bir şekilde yayından kaldırıldı.",
				type	: "success"
			},
			function(){
				window.location.reload(true);
			...
			sweetAlert({
				title	: "Başarılı",
				text 	: "Proje başarılı bir şekilde yayınlandı.",
				type	: "success"
			},
			function(){
				window.location.reload(true);
