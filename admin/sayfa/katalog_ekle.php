...
<div class="widget">
	<div class="widget-header transparent">
		<h2><strong>Yeni Katalog</strong> Ekle</h2>
	...
	<form role="form" class="form-horizontal"  id="forms" method="POST" action="ajax.php?p=fotograf_add"  enctype="multipart/form-data">
		...
		<div class="form-group">
			<label for="input-text" class="col-sm-2 control-label">Katalog Adı</label>
			<div class="col-sm-9">
				<input type="text" class="form-control" id="Baslik" name="fotograf_shortDesc" placeholder="xxx">
			</div>
		</div>						  
		<div class="form-group">
			<label class="col-sm-2 control-label">Ön Görsel Seçiniz</label>
			<div class="col-sm-9">
		...
		<div class="form-group">
			<label class="col-sm-2 control-label">Kataloğu Seçiniz</label>
			<div class="col-sm-9">
				<input type="file" class="btn btn-default" name="dosya2[]" id="dosya2[]" />
			</div>
		</div>
...
$(document).ready(function(event) {
	$('#forms').ajaxForm({
		uploadProgress: function(event, position, total, percentComplete) {
			swal({
				title: "Yükleniyor",
				text : "Katalog Yükleniyor. Lütfen Bekleyiniz",
				type : "info",
		...
		success: function(data) {
			if(data=="bos-deger"){
				...
				sweetAlert({
					title	: "Başarılı",
					text 	: "Katalog Başarılı Bir Şekilde Eklendi",
					type	: "success"
				},
				function(){
